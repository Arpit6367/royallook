...
function PuzzleCreator({
  ...
  const isEditMode = Boolean(puzzle)
  ...
  useEffect(() => {
    ...
    const puzzleStars = puzzle.data?.stars || []
    setStars([...puzzleStars])
    setInitialStars([...puzzleStars])
  ...
  // ───────────────── HELPERS ─────────────────
  const getTurnFromFen = (fenStr: string) => fenStr.split(' ')[1] || 'w'
  ...
  const handleImportPgn = () => {
    try {
      ...
      const baseFen = game.current.fen()
      setStartFen(baseFen)
      setFen(baseFen)
      setMoves(history)
      setMode('RECORD')
  ...
  const onSquareRightClick = (square: string) => {
    if (mode !== 'SETUP') return
    setStars(s =>
      s.includes(square) ? s.filter(x => x !== square) : [...s, square]
    )
  }
  ...
  const onSquareClick = (square: string) => {
    ...
    if (selectedTool === 'TRASH') game.current.remove(square)
    else game.current.put(selectedTool, square)
  ...
  const savePuzzle = async () => {
    ...
    const res = await fetch(
      isEditMode ? `/api/content/${puzzle!.id}` : '/api/content',
      {
    ...
    if (res.ok) {
      alert(isEditMode ? 'Puzzle Updated!' : 'Puzzle Created!')
      onBack()
    } else {
      alert('Save failed')
    }
  ...
  stars.forEach(sq => {
    customSquareStyles[sq] = {
      backgroundImage:
        'url("data:image/svg+xml;base64,")',
      backgroundRepeat: 'no-repeat',
      backgroundPosition: 'center',
      backgroundSize: '50%'
    }
  ...
  // ───────────────── UI ─────────────────
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 bg-white p-6 rounded-xl border h-full">
      <div className="lg:col-span-5 flex justify-center">
        <Chessboard
          position={fen}
          onPieceDrop={onPieceDrop}
      ...
      <div className="lg:col-span-7 space-y-4">
        <h2 className="text-2xl font-bold">
          {isEditMode ? 'Edit Puzzle' : 'New Puzzle'}
        </h2>
        ...
        {mode === 'SETUP' && (
          <>
            <input
              ...
              onChange={handleManualFenChange}
            />
            <button onClick={toggleMode}>Next: Record</button>
          </>
        )}

        {mode === 'RECORD' && (
          <>
            <div className="font-mono">{moves.join(' ')}</div>
            <input
            ...
            />
            <div className="flex gap-2">
              <button onClick={toggleMode}>Back</button>
              <button onClick={savePuzzle}>
