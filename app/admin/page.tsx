...
//              <h1 className="text-xl font-bold text-slate-800 tracking-tight">Chesspure Admin</h1>
//         </div>
...
// ==========================================
// 1. USER MANAGER (No Changes needed here based on request, kept for context)
// ==========================================
function UserManager() {
    const [users, setUsers] = useState<any[]>([])
    const [coaches, setCoaches] = useState<any[]>([])
...
// ==========================================
// 2. COURSE MANAGER (Added Delete Functionality)
// ==========================================
function CourseManager() {
  const [view, setView] = useState<'LIST' | 'EDIT_COURSE'>('LIST')
  const [courses, setCourses] = useState<any[]>([])
  ...
  const saveCourse = async () => {
    try {
        // If editingCourse has an ID, likely a PUT (update), else POST (create)
        // Adjust based on your API logic. Assuming POST handles both upsert or seperate PUT endpoint
                ...
                <div key={c.id} className="border rounded-xl p-5 hover:shadow-lg transition-shadow bg-gray-50 flex flex-col justify-between h-48 group">
                    <div>
                        <div className="flex justify-between items-start">
                            <span className={`text-[10px] font-bold px-2 py-1 rounded-full uppercase tracking-wide ${c.level === 'BEGINNER' ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'}`}>{c.level}</span>
                            <button onClick={(e) => { e.stopPropagation(); handleDeleteCourse(c.id); }} className="text-gray-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all p-1">
                                <Trash2 size={16} />
                            </button>
                        </div>
                        <h3 className="text-xl font-bold text-slate-800 mt-2 mb-1">{c.title}</h3>
...
// ==========================================
// 3. CURRICULUM MANAGER (Added Edit Puzzle Functionality)
// ==========================================
function CurriculumManager() {
    const [currentStage, setCurrentStage] = useState<string | null>(null)
    const [breadcrumbs, setBreadcrumbs] = useState<any[]>([])
...
// ==========================================
// 4. PUZZLE CREATOR (Updated: Edit Mode Support)
// ==========================================
function PuzzleCreator({ folderId, existingPuzzle, onBack }: { folderId: string, existingPuzzle?: any, onBack: () => void }) {
    ...
    // --- NEW: Load Existing Puzzle Data on Mount ---
    useEffect(() => {
        if (existingPuzzle) {
            ...
            // Load Stars (check for various data formats)
            const loadedStars = existingPuzzle.data?.stars || []
            setInitialStars(loadedStars)
            setStars(loadedStars)

            // Initialize board logic
            try {
                game.current.load(existingPuzzle.fen)
            } catch (e) { console.error("Error loading existing FEN", e) }

            // Default to record mode if everything is set
            setMode('RECORD')
        }
    }, [existingPuzzle])
   
    // Helper to extract active turn
    const getTurnFromFen = (fenStr: string) => {
        const parts = fenStr.split(' ')
        return parts.length > 1 ? parts[1] : 'w'
    }
    ...
    // 1. Sync Manual Input
    useEffect(() => {
        if (mode === 'SETUP') setManualFen(fen)
    }, [fen, mode])
    ...
    const toggleMode = () => {
      if (mode === 'SETUP') {
        ...
        // Going back to Setup - restore everything
        setMode('SETUP')
        setStartFen(null)
        setStars([...initialStars])
        setMoves([]) 
        // Reset board to initial setup position if it was set
        if (startFen) {
            try {
                game.current.load(startFen)
                setFen(startFen)
    ...
    const savePuzzle = async () => {
        ...
        // Determine method and payload (Edit vs Create)
        ...
        try {
            ...
            if(res.ok) {
                alert(existingPuzzle ? "Puzzle Updated Successfully!" : "Puzzle Saved Successfully!")
                onBack()
            } else {
                alert("Failed to save puzzle")
            }
