...
// ==========================================
// 1. COURSE MANAGER (FIXED SAVING & DRAG DROP)
// ==========================================
function CourseManager() {
  ...
  const addChapter = () => {
    ...
    game.current.load(newChapter.fen)
  }
  ...
  // Handle placing/removing pieces via clicks
  ...
  // Right click always removes pieces (Easy "drag off" alternative)
    ...
    <div className="bg-white rounded-xl shadow-lg border overflow-hidden flex flex-col h-[85vh]">
      ...
      <div className="bg-slate-800 text-white p-4 flex justify-between items-center shrink-0">
        ...
        <button 
            onClick={saveCourse} 
            disabled={loading}
            className="bg-green-600 hover:bg-green-700 disabled:bg-gray-400 px-6 py-2 rounded font-bold flex items-center gap-2"
        >
          {loading ? <Loader2 className="animate-spin"/> : <Save size={18}/>} Save Course
        </button>
      </div>
            ...
            <div className="h-full flex flex-col gap-6">
              ...
              <div className="grid grid-cols-2 gap-8 h-full">
                 <div className="flex flex-col gap-2">
                    ...
                    <textarea 
                      className="w-full flex-1 border rounded-lg p-4 resize-none focus:ring-2 focus:ring-orange-500 outline-none text-sm" 
                 ...
                 <div className="flex flex-col gap-2">
                    ...
                    <div className="border-4 border-slate-300 rounded-lg overflow-hidden shadow-sm aspect-square relative">
                        <Chessboard 
                          position={chapterFen} 
                          onPieceDrop={onDrop}
                        ...
                        />
                        {/* Setup Palette */}
                        <div className="absolute top-2 right-2 bg-white/90 p-2 rounded shadow border backdrop-blur-sm flex flex-col gap-1">
                             <div className="text-[10px] font-bold text-center text-slate-500 uppercase mb-1">Tools</div>
                             <div className="grid grid-cols-2 gap-1">
                                {['p','n','b','r','q','k'].map(p => (
                                    <button 
                                        key={'w'+p} 
                                        onClick={() => setSelectedTool({type: p, color: 'w'})} 
                                        className={`w-6 h-6 border rounded flex items-center justify-center font-serif font-bold text-xs bg-white text-black hover:bg-gray-100 ${selectedTool !== 'TRASH' && selectedTool?.type === p && selectedTool.color === 'w' ? 'ring-2 ring-blue-500' : ''}`}
                                    >
                                        {p.toUpperCase()}
                                    </button>
                                ))}
                             </div>
                             <div className="grid grid-cols-2 gap-1 border-t pt-1 mt-1">
                                {['p','n','b','r','q','k'].map(p => (
                                    <button 
                                        key={'b'+p} 
                                        onClick={() => setSelectedTool({type: p, color: 'b'})} 
                                        className={`w-6 h-6 border rounded flex items-center justify-center font-serif font-bold text-xs bg-slate-800 text-white hover:bg-slate-700 ${selectedTool !== 'TRASH' && selectedTool?.type === p && selectedTool.color === 'b' ? 'ring-2 ring-blue-500' : ''}`}
                                    >
                                        {p.toUpperCase()}
                                    </button>
                                ))}
                             </div>
                             <div className="border-t pt-1 mt-1 flex flex-col gap-1">
                                <button onClick={() => setSelectedTool('TRASH')} className={`p-1 rounded flex items-center justify-center text-red-600 hover:bg-red-50 ${selectedTool === 'TRASH' ? 'bg-red-100 ring-1 ring-red-500' : ''}`} title="Remove Piece">
                                    <Trash2 size={14}/>
                                </button>
                                <button onClick={() => { game.current.clear(); updateBoard() }} className="text-[10px] p-1 border rounded hover:bg-gray-100">Clear</button>
                                <button onClick={() => { game.current.reset(); updateBoard() }} className="text-[10px] p-1 border rounded hover:bg-gray-100">Start</button>
                             </div>
                        </div>
                    </div>
                    ...
                    <input 
                        className="text-[10px] font-mono border p-1 rounded bg-slate-50" 
                        value={chapterFen} 
                        readOnly 
                    />
                 </div>
...
// ... Rest of the components (UserManager, CurriculumManager, AnalysisBoard) remain unchanged ...
// Paste the rest of your original AdminDashboard code below here for UserManager, etc.
